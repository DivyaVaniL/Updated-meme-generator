...
function initCall() {
    ...
    $('a.restart').click(function (e) {
        ...
        //$('.camera-upload').slideDown();

        //$('#final-image, #share,#dsp, #signup,.chooseOption,.imageUploadedText').hide();
    ...
    function saveimage(isloaded, base_image) {
       ...
       // var colorsheight = canvas.width * colorsimage.height / colorsimage.width;
        ...
        if (photostage.getChildIndex(maskDragger) == -1) {
        	...
        	//photostage.removeChild(photodragger);

            	//context.drawImage(base_image, left, top, imgSize, imgSize, 0, 0, canvas.width, canvas.height, 0, 0);
        ...
        //context.drawImage(colorsimage, 0, 0, colorsimage.width, colorsimage.height, 0, drawposy, canvas.width, colorsheight, 0, 0);

        //context.drawImage(colorsimage, 0, 0, colorsimage.width, colorsimage.height, 0, 0, canvas.width, canvas.height, 0, 0);
...
function handleImage(e) {
    ...
    /*$('#upload label,.headerWrapper').addClass('disablebtn');*/
...
function updateShareLinks() {
    ...
    /*$("#tumblrshare").attr('href',tumblrShareURL);

    $("#tumblrshare").attr("title", tumblrShareTxt);*/
    ...
    //$("#instagramshare").attr('href', "download.php?guid="+memeGuid+"");
...
function shareLinks() {
    ...
    /*
     ...
     $(".homepage #instagramshare").click(function() {

     downloadCanvas(this, 'workarea', 'meme-image.png');
...
function clickOmniture(text) {
    ...
    s_dtm.prop1 = 'Mike Shinoda';

    s_dtm.eVar4 = 'Mike Shinoda';

    s_dtm.tl(this, 'o', 'Mike Shinoda Meme Generator:Landing:' + text);
...
function addImageToCanvas() {
    ...
    //stage.addChild(martifyImage);

    //stage.update();



    //stage.setChildIndex(maskDragger, stage.getNumChildren()-1);

    //finalstage.update();



    //bitmap.cache(0,0,670,670);
    ...
    //$("div.step.step2.hidden").removeClass("hidden");
...
function addMaskToCanvas() {
    ...
    maskImage = new createjs.Bitmap("./images/Colors.png");
    ...
    /*
     ...
     maskDragger.on("mousedown", function(evt) {
     ...
     maskDragger.on("pressmove", function(evt) {
    ...
    maskDragger.regX = (maskWidth / 2);

    maskDragger.regY = (maskHeight / 2);
    ...
    //maskDragger.x = (maskWidth / 2);

    //maskDragger.y = (maskHeight / 2);
    ...
    finalstage.addChild(maskDragger);
    ...
    $("#resizable").css("width", maskWidth + "px");

    $("#resizable").css("height", maskHeight + "px");
    ...
    $("#resizable").on("resize", function (event, ui) {
        ...
        var ratio = width / maskWidth;

        maskDragger.scaleX = maskDragger.scaleY = ratio;

        //maskDragger.regX = maskDragger.regX + (maskWidth / 2) * ratio ;

        //maskDragger.regY = maskDragger.regY + (maskHeight / 2) * ratio;
    ...
    /*$( "#resizable" ).on( "drag", function( event, ui ) {

    var x = ui.position.left;

    var y = ui.position.top;

    //maskDragger.x = Math.round(maskWidth / 2) + ( ( x + 2 ) / canvasScale);

    //maskDragger.y = Math.round(maskHeight / 2) + ( ( y + 1 ) / canvasScale);

    maskDragger.x = ( ( x + 2 ) / canvasScale);

    maskDragger.y = (( y + 1 ) / canvasScale);
    ...
    var eleWidth = maskWidth * ratio;

    var eleHeight = maskHeight * ratio;

    $(this).css("width", eleWidth+"px");

    $(this).css("height", eleHeight+"px");

    console.info(eleWidth);
...
$(document).ready(function () {
    ...
    $('#facebookshareremoved').click(function (e) {

        e.preventDefault();

        $('#twitter').hide();

        $('#facebook .caption textarea').val(decodeURIComponent(twitterShareTxt));
...
$("#facebooksharedummy").click(function () {
